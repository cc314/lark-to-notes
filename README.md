...
| `sync-events` | Ingest `im.message.receive_v1` and reaction create/delete NDJSON lines from stdin | Run `lark-cli event +subscribe …`, then pipe its stdout into `uv run lark-to-notes sync-events --db var/lark-to-notes.db --source-id dm:ou_xxx --json` (JSON includes `reaction_rows_inserted`, `reaction_benign_duplicate_replays` (canonical reaction id already stored), optional `--max-reactions-per-run` / `--max-reactions-per-source` cap counters (`reaction_cap_deferred`, `reaction_intake_run_id`), optional `--reaction-governance-version` / `--reaction-policy-version` stamped on new `message_reaction_events` rows, plus quarantine counters and last `payload_hash` / `reason_code` fields for failed reaction or receive-v1 rows; cap hits also append `reaction_intake_deferrals` rows) |
| `reaction-backfill` | Page `lark-cli im reactions list` over existing `raw_messages` for one `source_id` | `uv run lark-to-notes reaction-backfill --db var/lark-to-notes.db --source-id dm:ou_xxx --json` (uses `reaction_backfill_checkpoints` for `(created_at, message_id)` cursor + per-message `page_token` resume, `--min-interval-ms` pacing, optional `--max-messages`; inserts with `intake_path=backfill`; optional `--require-reaction-scopes`) |
...
- `reaction-backfill` walks `raw_messages` for a watched `source_id` and calls `lark-cli im reactions list` with resumable checkpoints in SQLite (`lw-pzj.6.2`); it shares the same `{parent-of---db}/lark-to-notes.runtime.lock` as `sync-events` / drain paths.
