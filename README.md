...
| `reaction-backfill` | Page `lark-cli im reactions list` over existing `raw_messages` for one `source_id` | `uv run lark-to-notes reaction-backfill --db var/lark-to-notes.db --source-id dm:ou_xxx --json` (uses `reaction_backfill_checkpoints` for `(created_at, message_id)` cursor + per-message `page_token` resume, `--min-interval-ms` pacing, optional `--max-messages`, optional `--since` / `--chat-id` scope on `raw_messages`, `--dry-run` to preview scan order without API or DB mutations; inserts with `intake_path=backfill`; optional `--require-reaction-scopes`) |
